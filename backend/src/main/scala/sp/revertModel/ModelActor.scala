        ...
        sendAnswer(updH, APISP.SPACK())
          b match {
            ...
            case k: APIModel.RevertModel if h.to == id.toString =>
              if(state.version != k.toVersion) { // Delete history to point of reversion, set current state to the previous version state
                state = ModelState(k.toVersion, state.history(k.toVersion)._2.toMap, state.history.filterKeys(_ <= k.toVersion), state.attributes.addTimeStamp(), state.name)
                sendAnswer(updH, APIModel.ModelUpdate(id, state.version, state.items.size)) // Update model
              }
...
trait ModelLogicRevert {
  ...
  case class ModelState(version: Int, idMap: Map[ID, IDAble], history: Map[Int, (SPAttributes, List[(ID, IDAble)])], attributes: SPAttributes, name: String){
  ...
  val initialHistory = (SPAttributes("info"->s"Model created."), List())
  ...
  def getModelHistory = APIModel.ModelHistory(id, state.history.map(m => (m._1 -> m._2._1)).toList.sortWith(_._1 > _._1))
  ...
  def makeModelUpdate(diff: ModelDiff) = {
    ...
    def updateState(diff: ModelDiff) = {
      ...
      val diffMap = state.history + (version -> (diff.diffInfo, allItems.toList))
