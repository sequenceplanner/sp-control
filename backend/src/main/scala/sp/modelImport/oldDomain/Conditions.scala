package sp.modelImport.oldDomain
...
object Action {
  def parseStr(str: String, idables: List[IDAble] = List()): Option[Action] = {
    val propParser = sp.modelImport.oldDomain.logic.ActionParser(idables)
...
object Proposition {
  def parseStr(str: String, idables: List[IDAble] = List()): Option[Proposition] = {
    val propParser = sp.modelImport.oldDomain.logic.PropositionParser(idables)
    propParser.parseStr(str).right.toOption
  }
