...
trait JsonImplicit extends JsonDerived {
  ...
  implicit val fHierarchyNode: JSFormat[HierarchyNode] = Json.format[HierarchyNode]
  ...
  /* implicit lazy val HierarchyRootRead: JSReads[HierarchyNode] = (
     (JsPath \ "item").read[ID] and
       (JsPath \ "children").read[List[HierarchyNode]] and
       (JsPath \ "id").read[ID]
     )(HierarchyNode.apply _)

   implicit lazy val HierarchyRootwrites: JSWrites[HierarchyNode] = (
     (JsPath \ "item").write[ID] and
       (JsPath \ "children").write[List[HierarchyNode]] and
       (JsPath \ "id").write[ID]
     )(unlift(HierarchyNode.unapply)) */

  //implicit val fHierarchyRoot: JSFormat[HierarchyRoot] = Json.format[HierarchyRoot]

  implicit lazy val readHierarchyRoot: Reads[HierarchyRoot] = new Reads[HierarchyRoot] {
    def reads(json: JsValue): JsResult[HierarchyRoot] = {
      val idable = extrIDable("HierarchyRoot", json)
      val children = (JsPath \ "children").read[List[HierarchyNode]].reads(json).getOrElse(List())
      idable.map{case (n, id, attr) => HierarchyRoot(n, children, attr, id)}
    }
  }
  implicit lazy val writeHierarchyRoot: Writes[HierarchyRoot] = new Writes[HierarchyRoot] {
    override def writes(o: HierarchyRoot): SPValue = {
      wwIDAble("HierarchyRoot", SPAttributes("children"->o.children), o)
    }
  }
  ...
  implicit lazy val readIDAble: Reads[IDAble] = new JSReads[IDAble] {
    override def reads(json: SPValue): JsResult[IDAble] = {
      json.validate[Operation] orElse
        ...
        json.validate[Struct] orElse
      json.validate[HierarchyRoot]
    }
  }
  implicit object WriteIDAble extends Writes[IDAble]  {
    override def writes(o: IDAble): SPValue = {
      o match {
        ...
        case x: HierarchyRoot=> Json.toJson(x)(writeHierarchyRoot)
      }
    }
  }
