...
trait MakeASop {
  ...
  def cleanSop(sop: SOP): SOP = {
    sop match {
      ...
      case x:Parallel if x.sop.size == 1 && x.sop.head.isInstanceOf[Sequence] =>
        cleanSop(Sequence(x.sop.head.sop))
      case x:Sequence if x.sop.size == 1 && x.sop.head.isInstanceOf[Sequence] =>
        cleanSop(x.sop.head)
      case _ => sop.modifySOP(sop.sop.map(cleanSop))
    }
  }
  ...
  def identifySOPRelation(sop1: SOP, sop2: SOP, relations: Map[Set[ID], SOP]): SOP = {
    ...
    val relationBetweenPairs = (for {
      ...
      if (s1 == s2) Other(List())
        ...
        relations(Set(s1, s2)) match {
          case x: Sequence => if (x.sop.head.asInstanceOf[OperationNode].operation == s1) Sequence(List(sop1, sop2)) else Sequence(List(sop2, sop1))
          case x: SometimeSequence => if (x.sop.head.asInstanceOf[OperationNode].operation == s1) SometimeSequence(List(sop1, sop2)) else SometimeSequence(List(sop2, sop1))
          case x: SOP => {x.modifySOP(List(sop1, sop2)) }
        }
      }
    }).toSet
    ...
    relationBetweenPairs.toList match {
      case x :: Nil => x
      case x :: y :: Nil if instanceOfSequence(x, y) => Sequence(List(x.sop.head, x.sop.tail.head))
      case _ => Other(List(sop1, sop2))
    }
  ...
  def extractOps(sop: SOP): List[ID] = {
    sop match {
      case x: OperationNode => List(x.operation) // does not need to dig since the op is included in the relationMap
  ...
  def sequencify(sops: Seq[SOP], relations: Map[Set[ID], SOP], base: SOP = EmptySOP): List[SOP] = {
    val updSops = sops map{
      ...
      case s: SOP => s.modifySOP(sequencify(s.sop, relations, base))
    }
  ...
  private def createSequenceSOP(seq: Seq[Node], rels: Map[Set[ID], SOP]): SOP = {
      ...
      left.toList match {
        ...
        case x::xs => {
          identifySOPRelation(x.s, prev, rels) match {
            case s: SometimeSequence => getSomtimeSeq(xs, x.s, result.addChildren(Seq(x.s)))
    ...
    def getAlwaysSeq(left: Seq[Node], result: SOP): SOP = {
      left.toList match {
        case Nil => result
        case x::Nil => result.addChildren(Seq( x.s))
        case x::xs => {
          val rel = identifySOPRelation(x.s, xs.head.s, rels)
          // val rel = relations(Set(x.s, xs.head.s))
          rel match {
            case s: SometimeSequence => {
              val someTime = SometimeSequence(List(x.s))
              val pair = getSomtimeSeq(xs, x.s, someTime)
              getAlwaysSeq(pair._2, result.addChildren(Seq( pair._1)))
            }
            case s: Sequence => getAlwaysSeq(xs, result.addChildren(Seq(x.s)))
          }
        }
      }
    }

    val res = getAlwaysSeq(seq, Sequence(List()))
  ...
  def getOpsNames(s: SOP): String = {
    s match {
      case h: OperationNode => h.operation.toString()
