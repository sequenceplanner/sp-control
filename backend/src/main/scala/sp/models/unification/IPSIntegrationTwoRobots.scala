package sp.models.unification.ipsintegrationtworobots
...

{
  ...
  val gotoPositions = UR.poses.filter(_!="UNKNOWN").map { p =>
    ...
    val ur2 = c("pre", s"ur2.actPos != '$p' && ur2.refPos != '$p'")
...

{
  ...
  val gotoPositions = UR.poses.filter(_!="UNKNOWN").map { p =>
    val of = UR.needsOf.contains(p)
    val ofc = if(of) c("pre", s"ur2.ofAttached") else c("pre", "true")
    val of1 = if(p == "OF_1_TIGHTENED") c("isFinished", "true", "of2 := true") else c("isFinished", "true")
    val ur1 = c("pre", s"ur.actPos != '$p' && ur.refPos != '$p'")

    val source = UR.canOnlyGoFrom.get(p).map(source => c("pre", s"ur2.actPos == '$source'")).getOrElse(c("pre", "true"))

    val op = o(s"ur2.goto$p", "ur2.moveToPos", "ur2")(
      c("pre", s"ur2.actPos != '$p' && ur2.refPos != '$p'", s"ur2.refPos := '$p'"),
      ur1,
      ofc,
      source,
      c("isFinished", "true"),
  ...
  val attach = o("ur2.attach")(
    c("pre", s"!ur2.ofAttached && ur2.actPos == 'ATTACH_OF' && ur2.refPos == 'ATTACH_OF'", "ur2.ofAttached := true"),
    c("isExecuting", "false"),
    c("isFinished", "ur2.ofAttached")
  )

  val detach = o("ur2.detach")(
    c("pre", s"ur2.ofAttached && ur2.actPos == 'ATTACH_OF' && ur2.refPos == 'ATTACH_OF'", "ur2.ofAttached := false"),
    c("isExecuting", "false"),
    c("isFinished", "!ur2.ofAttached")
  )
}
