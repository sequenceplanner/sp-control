package spgui.widgets.newSopmaker
  ...
  private class Backend($: BackendScope[Unit, State]) {
    ...
    def onMount() = Callback{
      SPGUICircuit.subscribe(SPGUICircuit.zoom(z => z.draggingState.latestDropEvent.get)){
           // println("hej")
        x => if(x.value != null){
          println("Got a drop event!  : "   +  x)
          $.modState(
            s => State(insertSop(s.sop, x.value.targetId, x.value.droppedId )._1)
          )
        }.runNow()
      }
    }
    ...
    def insertSop(root: SOP, targetId: UUID, sopId: UUID): (SOP, Boolean) = {
      var result = SOP()
      var done = false

      if(root.nodeID == targetId) { // found what we were looking for, add it to the SOP
        result =
        root match {
          case r: Parallel =>
          ...
          case r: OperationNode =>
            Parallel(sop = List(r, findSop($.state.runNow().sop, sopId))) // I would prefer Sequence...
        }
        done = true
      }
      else { // did not find the thing, keep looking
        result =
        root match {
          case r: Parallel =>
            Parallel(sop = r.sop.collect{case e => if(!done) {val res = insertSop(e, targetId, sopId); if(res._2) done = true; res._1} else  e })
          case r: Sequence =>
            Sequence(sop = r.sop.collect{case e => if(!done) {val res = insertSop(e, targetId, sopId); if(res._2) done = true; res._1} else e })
          case r: OperationNode =>
            r
        }
      }
