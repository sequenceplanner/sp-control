...
object TestModelInControl {
  def getTestModel: List[IDAble] = {
    ...
    // TODO: Fix a good struct DSL!
    val struct1 = Struct("struct1", List(
      StructNode(o1.id, Some(t1.id)),
      StructNode(o2.id, Some(t2.id)),
      StructNode(o3.id, Some(t2.id)),
      StructNode(t3.id, Some(t2.id)),
      StructNode(o4.id, Some(t3.id)),
      StructNode(t4.id, Some(o4.id)),
      StructNode(t1.id, None),
      StructNode(t2.id, None),
      StructNode(sop1.id, Some(o4.id))
    ))
    val struct2 = Struct("struct2", List(
      StructNode(sop2.id, Some(o1.id)),
      StructNode(o1.id, None)
    ))

    List(o1, o2, o3, o4, t1, t2, t3, t4, sop1, sop2, struct1)
  }
}
